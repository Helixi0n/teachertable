        ...
        while True:
            try:
                ...
                if current_time.hour == 9 and current_time.minute == 0:
                    event_list = session.query(Event).filter(Event.date_time_event >= current_date_time).all()
                    ...
                    for event in event_list:
                        ...
                        reminder_week_date = reminder_date_week.date()
                        reminder_day_date = reminder_date_day.date()

                        if current_date == reminder_week_date:
                            msg = f'Напоминаем, что через неделю у вас состоится событие "{event.text}"\nДата: {event.date_time_event.strftime("%d.%m.%Y %H:%M")}'
                            try:
                                self.bot.send_message(
                                    event.teacher_id,
                                    msg
                        ...
                        elif current_date == reminder_day_date:
